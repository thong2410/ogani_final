...
@section('content')
<h3 class="mb-4">@lang('admin.category.categories')</h3>
@if (session('message'))
...
<form action="{{route('admin.category.index')}}" method="GET">
  <div class="row">
    ...
    <div class="col-2 col-xs-12 mb-2">
        ...
        @if(isset($input['keyword']))<a href="{{route('admin.category.index')}}"><button class="btn btn-info" type="button"><i class="fas fa-reply-all"></i></button></a>@endif
    </div>
  </div>
</form>
<hr/>
    ...
    <table class="table table-bordered bg-white">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <form action="{{ route('admin.category.destroy', $category->cate_id) }}" method="POST"
                        class="form-inline">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <button type="submit" class="btn btn-sm btn-outline-danger mr-1"><i class="fa fa-trash"
                                aria-hidden="true"></i></button>
                        <a href="{{ route('admin.category.edit', $category->cate_id) }}">
                ...
                <tr>
                    ...
                    <td>
                        <form action="{{ route('admin.category.destroy', $subCategory->cate_id) }}" method="POST"
                            class="form-inline">
                            <input type="hidden" name="_method" value="DELETE">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            <button type="submit" class="btn btn-sm btn-outline-danger mr-1"><i class="fa fa-trash"
                                    aria-hidden="true"></i></button>
                            <a href="{{ route('admin.category.edit', $subCategory->cate_id) }}">
                                <button type="button" class="btn btn-sm btn-outline-success"><i class="fa fa-pen"
                                        aria-hidden="true"></i></button>
                            </a>
                        </form>
